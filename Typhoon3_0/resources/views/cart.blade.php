    ...
    <h2>Your products: </h2>
        ...
        @if(session('cart'))
            @foreach(session('cart') as $id => $details)
                ...
                @php $names = $names  . "X" . $details['quantity'] . " " . $details['name'] . ", " @endphp
...
@endforeach
    ...
    <tfoot>
        <tr>
            <td colspan="5" class="text-right"><p style="color:white; margin-left: auto; margin-right: 20px; margin-top: auto; margin-bottom: auto; font-weight: bold; font-size:30px">Razem: Total ${{ $total }}</p></td>
        </tr>
        <tr>
            <td colspan="5" class="text-right">
            <form action="{{route('tests', $names)}}">
